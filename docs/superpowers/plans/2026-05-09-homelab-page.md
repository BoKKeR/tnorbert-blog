# Homelab Page Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace `src/app/(frontend)/homelab/page.tsx` with a Kubernetes-focused page covering nodes, Ceph storage, Flux+Ansible management, Grafana monitoring, and traffic flow.

**Architecture:** Single static Next.js page component. All data is declared as typed constants at the top of the file; JSX sections below consume them. No client components, no new dependencies, no API calls.

**Tech Stack:** Next.js 15 (App Router), TypeScript, Tailwind CSS. Follows existing blog design tokens (cream/ink/orange/blue palette, `font-serif` headings, `font-mono` labels, `border-border` borders, `rounded-sm` corners).
...
| Action | Path |
|--------|------|
| Modify (full replace) | `src/app/(frontend)/homelab/page.tsx` |

No new files. No other files touched.

---

### Task 1: Skeleton — metadata, types, empty sections

**Files:**
- Modify: `src/app/(frontend)/homelab/page.tsx`

- [ ] **Step 1: Replace the file with a typed skeleton**

Write the following content to `src/app/(frontend)/homelab/page.tsx`:
...
```bash
git add src/app/\(frontend\)/homelab/page.tsx
git commit -m "feat(homelab): skeleton with types and data constants"
```

---

### Task 2: Cluster Overview — stat pills

**Files:**
- Modify: `src/app/(frontend)/homelab/page.tsx`

- [ ] **Step 1: Replace `{/* TODO: sections go here */}` with the cluster overview section**
...
```bash
git add src/app/\(frontend\)/homelab/page.tsx
git commit -m "feat(homelab): cluster overview section with stat pills"
```
...
- [ ] **Step 1: Replace `{/* TODO: remaining sections */}` with the node grid section**
...
```bash
git add src/app/\(frontend\)/homelab/page.tsx
git commit -m "feat(homelab): node grid with specs, badges, and stat rows"
```

---

### Task 4: Storage — Ceph Cluster

**Files:**
- Modify: `src/app/(frontend)/homelab/page.tsx`

- [ ] **Step 1: Replace `{/* TODO: remaining sections */}` with the Ceph storage section**
...
```bash
git add src/app/\(frontend\)/homelab/page.tsx
git commit -m "feat(homelab): Ceph storage section with TinyRiser explanation and drive inventory"
```

---

### Task 5: Management — Flux + Ansible

**Files:**
- Modify: `src/app/(frontend)/homelab/page.tsx`

- [ ] **Step 1: Replace `{/* TODO: remaining sections */}` with the management section**
...
```bash
git add src/app/\(frontend\)/homelab/page.tsx
git commit -m "feat(homelab): management section — Flux GitOps and Ansible provisioning"
```

---

### Task 6: Monitoring — Grafana + Prometheus + Loki

**Files:**
- Modify: `src/app/(frontend)/homelab/page.tsx`

- [ ] **Step 1: Replace `{/* TODO: remaining sections */}` with the monitoring section**
...
```bash
git add src/app/\(frontend\)/homelab/page.tsx
git commit -m "feat(homelab): monitoring section — Prometheus, Grafana, Loki, Alertmanager"
```
...
- [ ] **Step 1: Replace `{/* TODO: remaining sections */}` with the traffic flow section**
...
Note: remove `{/* TODO: remaining sections */}` — this is the last section before the footer.

- [ ] **Step 2: Verify TypeScript compiles and lints clean**
...
```bash
git add src/app/\(frontend\)/homelab/page.tsx
git commit -m "feat(homelab): traffic flow section with Cloudflare → Envoy → Service → Pod steps"
```
...
**Files:**
- Read: `src/app/(frontend)/homelab/page.tsx`

- [ ] **Step 1: Confirm no TODO comments remain in the file**

```bash
grep -n "TODO" src/app/\(frontend\)/homelab/page.tsx
```
...
Open `http://localhost:3000/homelab` and check:
- Back link renders
- Stat pills row visible under header prose
- 6 node cards in 2-column grid; worker cards have `25GbE` badge; controller cards show `(schedules pods)`
- Ceph section has TinyRiser paragraph + drive inventory list
- Management section: two side-by-side cards (Flux / Ansible)
- Monitoring section: 4 rows with amber category badges
- Traffic flow: 4-step numbered list with connector lines + CoreDNS note at bottom
- No `{/* TODO */}` markers visible in page source

- [ ] **Step 4: Final commit**

```bash
git add src/app/\(frontend\)/homelab/page.tsx
git commit -m "feat(homelab): complete homelab page — nodes, Ceph, Flux, Grafana, traffic flow"
```
